...
- 2026-09-23 の実測（本 slice が変える現状）: `AuthViewModel.onUnauthorized` は後始末を呼ばない（`AuthViewModel.kt:232`「order:12」）。`logout()` は `onSubjectLeave()` の完了後に `sessionStore.clear()`（`:342-350`。テスト `AuthViewModelTest.kt:704` が pin）。`CleanupIncomplete` は未実装（grep 0）。`AudioCacheManager` は `{baseDir}/audio/{id}.mp3` の平置き（`removeAll()` は `AudioCacheManager.kt:81`）。`UserResponse` に `user_id` 無し。`_authState.value =` の書込は `AuthViewModel.kt` に 8 箇所。`FcmTokenRegistrar.onLogout()`（`:43-47`）が `unregisterDeviceToken` を呼ぶ。`cancelDownloadsAndClearCache` の一致は main に 12（`AppContainer.kt:256, 257, 259, 269, 274`・`AuthViewModel.kt:49, 55`（コメント）・`PodcastViewModel.kt:105, 108, 187, 232, 235`）。
...
- **参照 0 件**（対象集合 = `app/src/main` の全 Kotlin。コメントも含む。除外 = `docs/`・`build/`・`app/src/test`）: `grep -rn 'removeAll()\|cancelDownloadsAndClearCache' app/src/main` = 0。2026-09-23 実測の分母 = `removeAll()` 2（`AudioCacheManager.kt:81` 宣言・`FileSystem.kt:8` コメント）＋ `cancelDownloadsAndClearCache` 12（上記「実測」）。コメント 3 箇所（`FileSystem.kt:8`・`AuthViewModel.kt:49, 55`）は新名へ書き換える。`grep -rn '"audio"' app/src/main` の一致は `network/AudioCacheManager.kt` のみ。`grep -rn '_authState.value =' app/src/main/java/com/rioikeda/newslisten/auth/AuthViewModel.kt` = 1（遷移関数）。
...
- grep oracle 回帰なし: `error("` = 0。`code == 401` の一致は `network/AuthInterceptor.kt:48` と `network/OkHttpApiClient.kt:459` の 2 箇所から増えない（`logout(token)` の 401 を新たに比較しない）。`currentPodcast` の件数は本 slice で変えない。

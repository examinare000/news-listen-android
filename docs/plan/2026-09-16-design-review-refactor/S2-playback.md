...
- **Selection Gate（pending の間は現行値で pin）**:
  - **SG-X1**（完聴時にサーバーへ送る位置。候補 (a) 0 / (b) duration）: pending の間は Android の**現行挙動（完聴時点の再生位置 `positionSeconds` をそのまま送る）で pin**する（CI-T8 の送信値の部分だけ現行維持）。SG-R14 の「明示的に duration を送る」は本 slice では実装せず、SG-X1 が (b) で satisfied になってから差分 PR として起票する。
  - **SG-X3**（主体離脱で cleanup 完了を待つか）: pending の間は**現行の「待つ」順序を維持**する。
  - **SG-X4**（一時停止中も位置を周期送信するか）: pending の間は**現行の「送る」を維持**する（CI-T19 の送信条件はこれとは別に「session が prepare 済みか」で判定する）。
  - いずれも「pending を選択済みとして扱わない」（共有仕様 §6.7）。着手を止めるのは該当契約の値のみで、S2 全体は止めない。
...
8. **`podcast/PodcastViewModel`（PlaybackCoordinator）**: `startEpisode`（source 解決 → `PlaybackSession.start` → `prepare → setSpeed(既定速度) → seekTo(resume, resume > 0 のとき) → play`）、`onEnded`（`markCompleted` 1 セッション 1 回 → 位置同期 1 回（値は現行どおり完聴時点の `positionSeconds`。SG-X1 pending のため duration への変更はしない）→ `queue.advance` → 次があれば `startEpisode`、無ければ `Stopped`）、`stopForSubjectLeave()`（位置同期 1 回 → `playerController.stop()` → `queue = PlaybackQueue()` → `NothingPlaying`。`AppContainer.onSubjectLeave` から呼ぶ）、位置同期の事前条件（`Starting`/`Active`/`Completed` のときだけ送る）を実装する。
...
| CI-T8 | 完聴時の順序 = `markCompleted` → `updatePlaybackPosition(…)` → `advance`。同一 episode の `markCompleted` は 1 セッション 1 回。**送信値は SG-X1 pending のため現行（完聴時点の `positionSeconds`）を pin し、順序と 1 回抑止だけを契約にする** | T-T8: `FakePodcastApi` の呼出列 |
...
| CI-T19 | 位置同期（15 秒・停止直前・完聴時）は session が `Starting`/`Active`/`Completed` のときだけ送る。`Errored`/`Stopped`/`NothingPlaying` では送らない | T-T19: `Errored(FetchFailed)` 後に timer を進めても `FakePodcastApi.updatePlaybackPosition` が呼ばれない |
...
7. T-T8（完聴順序、SG-X1 pin）・T-T19（位置同期の送信条件）→ RED → 実装 → GREEN。
...
- CI-T8（完聴時の送信値）のテストに「SG-X1 pending のため現行の送信値（完聴時点の `positionSeconds`）を pin」の旨がコメントで明記されている。
...
## 禁止事項 / scope 外
- SG-X1 が satisfied になるまで、完聴時の送信値を変えない（duration への変更は gate 確定後の差分 PR）。
- SG-X3・SG-X4 の値の変更（cleanup 完了待ちをやめる・一時停止中送信をやめる）はしない。

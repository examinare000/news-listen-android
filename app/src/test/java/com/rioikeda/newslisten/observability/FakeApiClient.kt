...
class FakeApiClient(
    ...
    override suspend fun login(username: String, password: String): LoginResponse =
        error("login is out of scope for observability tests")

    override suspend fun logout() = error("logout is out of scope for observability tests")

    override suspend fun me(): UserResponse = error("me is out of scope for observability tests")

    override suspend fun fetchFeed(filter: String): FeedResponse =
        error("fetchFeed is out of scope for observability tests")

    override suspend fun starArticle(id: String, request: StarRequest): ActionResponse =
        error("starArticle is out of scope for observability tests")

    override suspend fun dismissArticle(id: String): ActionResponse =
        error("dismissArticle is out of scope for observability tests")

    override suspend fun fetchPodcasts(): PodcastListResponse =
        error("fetchPodcasts is out of scope for observability tests")

    override suspend fun fetchPodcast(id: String): PodcastResponse =
        error("fetchPodcast is out of scope for observability tests")

    override suspend fun updatePlaybackPosition(id: String, positionSeconds: Double): PodcastResponse =
        error("updatePlaybackPosition is out of scope for observability tests")

    override suspend fun fetchPreferences(): PreferencesResponse =
        error("fetchPreferences is out of scope for observability tests")

    override suspend fun downloadAudio(url: String): ByteArray =
        error("downloadAudio is out of scope for observability tests")

    override suspend fun registerDeviceToken(token: String, platform: String) =
        error("registerDeviceToken is out of scope for observability tests")

    override suspend fun unregisterDeviceToken(token: String, platform: String) =
        error("unregisterDeviceToken is out of scope for observability tests")

    override suspend fun fetchSources(): RssSourcesResponse =
        error("fetchSources is out of scope for observability tests")

    override suspend fun createSource(name: String, url: String): RssSourcesResponse =
        error("createSource is out of scope for observability tests")

    override suspend fun updateSource(oldUrl: String, name: String, url: String): RssSourcesResponse =
        error("updateSource is out of scope for observability tests")

    override suspend fun deleteSource(url: String): RssSourcesResponse =
        error("deleteSource is out of scope for observability tests")

    override suspend fun fetchFeaturedSites(): FeaturedSitesResponse =
        error("fetchFeaturedSites is out of scope for observability tests")

    override suspend fun updatePreferences(defaultDifficulty: String?, defaultPlaybackSpeed: Double?): PreferencesResponse =
        error("updatePreferences is out of scope for observability tests")

    override suspend fun fetchGenerationQuota(): GenerationQuotaResponse =
        error("fetchGenerationQuota is out of scope for observability tests")

    override suspend fun fetchListeningStreak(): ListeningStreakResponse =
        error("fetchListeningStreak is out of scope for observability tests")

    override suspend fun updateProfile(displayName: String): UserResponse =
        error("updateProfile is out of scope for observability tests")

    override suspend fun changePassword(currentPassword: String, newPassword: String) =
        error("changePassword is out of scope for observability tests")

    override suspend fun listSessions(): SessionsListResponse =
        error("listSessions is out of scope for observability tests")

    override suspend fun revokeSession(id: String) =
        error("revokeSession is out of scope for observability tests")
    ...
    override suspend fun passkeyRegisterOptions(): PasskeyOptionsResponse =
        error("passkeyRegisterOptions is out of scope for observability tests")

    override suspend fun passkeyRegisterVerify(challengeId: String, credential: JsonObject) =
        error("passkeyRegisterVerify is out of scope for observability tests")

    override suspend fun passkeyLoginOptions(username: String?): PasskeyOptionsResponse =
        error("passkeyLoginOptions is out of scope for observability tests")

    override suspend fun passkeyLoginVerify(challengeId: String, credential: JsonObject): LoginResponse =
        error("passkeyLoginVerify is out of scope for observability tests")

    override suspend fun listPasskeyCredentials(): PasskeyCredentialsListResponse =
        error("listPasskeyCredentials is out of scope for observability tests")

    override suspend fun deletePasskeyCredential(credentialId: String) =
        error("deletePasskeyCredential is out of scope for observability tests")
}
